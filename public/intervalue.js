...
angular.module('copayApp.services').factory('backButton', function($log, $rootScope, gettextCatalog, $deepStateRedirect, $document, $timeout, go, $state, lodash, profileService) {
	...
	function back() {
		...
		else if ( root.showPassword ) {
            $rootScope.$emit('closePassword');
		}
...
angular.module('copayApp.services').factory('correspondentListService', function($state, $rootScope, $sce, $compile, configService, storageService, profileService, go, lodash, $stickyState, $deepStateRedirect, $timeout, gettext) {
            ...
            walletGeneral.readMyAddresses(function(arrMyAddresses){
                ...
                for (var i in messages) {
                    ...
                    if (last_msg_ts && last_msg_ts.getDay() != msg_ts.getDay()) {
                        messageEvents.unshift({type: 'system', bIncoming: false, message: "<span style='color:#fff;background-color:#cad1d9;padding: 4px 10px;border-radius: 2px;'>" + last_msg_ts.toDateString() + "</span>", timestamp: Math.floor(msg_ts.getTime() / 1000)});
                    }
                    last_msg_ts = msg_ts;
                ...
                if (historyEndForCorrespondent[correspondent.device_address] && messageEvents.length > 1) {
                    messageEvents.unshift({type: 'system', bIncoming: false, message: "<span style='color:#fff;background-color:#cad1d9;padding: 4px 10px;border-radius: 2px;'>" + (last_msg_ts ? last_msg_ts : new Date()).toDateString() + "</span>", timestamp: Math.floor((last_msg_ts ? last_msg_ts : new Date()).getTime() / 1000)});
                }
                $timeout(function(){
                    $rootScope.$digest();
                });
    ...
    function checkAndInsertDate(messageEvents, message) {
        ...
        if (last_msg_ts.getDay() != msg_ts.getDay()) {
            messageEvents.push({type: 'system', bIncoming: false, message: "<span style='color:#fff;background-color:#cad1d9;padding: 4px 10px;border-radius: 2px;'>" + msg_ts.toDateString() + "</span>", timestamp: Math.floor(msg_ts.getTime() / 1000)});
        }
    }
  ...
  function($timeout) {
    ...
    */
    ...
    var settings = {
      ...
      localStorage: false,
      html5Mode: false,
      html5DefaultIcon: 'img/icons/icon-white.ico'
    };

    function html5Notify(icon, title, content, ondisplay, onclose) {
      if (window.webkitNotifications && window.webkitNotifications.checkPermission() === 0) {
        if (!icon) {
          icon = 'img/icons/icon-white.ico';
        }
...
angular.module('copayApp.services')
    .factory('profileService', function profileServiceFactory($rootScope, $location, $timeout, $filter, $log, lodash, storageService, bwcService, configService, pushNotificationsService, isCordova, gettext, gettextCatalog, nodeWebkit, uxLanguage) {
        ...
        root.formatAmountWithUnitNoSeparators = function (amount, asset, opts) {
            return root.formatAmountWithUnit(amount, asset, opts).replace(",", "");
        };
...
angular.module('copayApp.controllers').controller('wordsController', function ($rootScope, $scope, $timeout, profileService, go, gettext, confirmDialog, notification, $log, isCordova, storageService,configService,uxLanguage) {
	...
	if(lg==='zh_CN'){
        msg = gettext('您确定要删除备份文档吗？');
        successMsg = gettext('备份文档已删除');
	}
